...
- **数据性质**：价格/链接/发货时效以平台最新页面为准，**上架前请人工复核**。
- **避开爱善天使（俏妃母公司）旗下自有产品**：程序内置**双重黑名单**——品牌黑名单（俏妃/小黄巾/福御医/逸红颜/护宫福/优感觉/悦年轻/悦轻松/紫色风铃/婕妙/卫蕾/小良知/胶贵人/爱善天使）+ 品类黑名单（卫生巾/安心裤/湿巾/纸巾/私护套盒/艾灸贴/小罐灸/温宫调经·净腔养巢套盒/拔湿膏贴/三参源气饮等），并有一道「确定性兜底过滤」，确保选中产品不与公司自有货冲突。
- **产品带图**：网页每个产品卡与本周主推均显示商品主图（自动通过 SerpAPI 搜图，每日上限 18 次控额度；搜不到则回退成带产品名+平台色的精美占位图，不打乱排版）。
- **每天自动跑**：通过 **Windows 任务计划**（不是 WorkBuddy 自动化），在本机 09:00（工作日）自动执行。每日选品数量由 `config.json` 的 `business.max_products` 控制（当前 20）。
...
├── config.json          # 所有配置（API 密钥、平台、每日选品数、爱善天使排除黑名单、调度时间、企微 webhook）
├── main.py              # 入口
